...
export class ContractAirVoyage extends ContractBase {
    ...
    public registerEvents(contractIns: any, currentGameId?: any) {
        ...
        contractIns.on('GameCreated', (gameId: any, player: any) =>
            gameEventListenerManager.addEventToQueue(GameEventContractAirVoyageGameCreated.event, [
                gameId,
                player,
            ])
        );
        contractIns.on('GameJoined', (gameId: any, player: any) =>
            gameEventListenerManager.addEventToQueue(GameEventContractAirVoyageGameJoined.event, [
                gameId,
                player,
            ])
        );
        contractIns.on('GameStarted', (gameId: any) =>
            gameEventListenerManager.addEventToQueue(GameEventContractAirVoyageGameStarted.event, [
                gameId,
            ])
        );
        contractIns.on('DiceRolled', (gameId: any, player: any, dice: any) =>
            gameEventListenerManager.addEventToQueue(GameEventContractAirVoyageDiceRolled.event, [
                gameId,
                player,
                dice,
            ])
        ...
        contractIns.on('PieceFinished', (gameId: any, player: any, pieceIndex: any) =>
            gameEventListenerManager.addEventToQueue(
