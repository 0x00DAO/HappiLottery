...
const DeployContractName = 'AirVoyageGameEntity';
const contractAddress = ContractDeployAddress.AirVoyageGameEntity;
...
async function afterDeploy(contract: Contract) {
  ...
  await contract.grantRole(
    await contract.COMPONENT_WRITE_ROLE(),
    ContractDeployAddress.AirVoyage
  );

  await contract.grantRole(
    await contract.COMPONENT_WRITE_ROLE(),
    ContractDeployAddress.AirVoyageGameMoveSystem
  );
}
